...
} /* exports */ /* eslint-disable no-self-assign */
...
ApplyShim.prototype['_separator'] = MIXIN_VAR_SEP; /* eslint-enable no-self-assign */
...
CustomStyleInterface.prototype['processStyles'] = CustomStyleInterface.prototype.processStyles; /* eslint-enable no-self-assign */
...
if (!window.ShadyCSS || !window.ShadyCSS.ScopingShim) {
  ...
  let CustomStyleInterface = window.ShadyCSS && window.ShadyCSS.CustomStyleInterface; /** @suppress {duplicate} */
  window.ShadyCSS = {
    /**
     * @param {!HTMLTemplateElement} template
     * @param {string} elementName
...
var styleGather = {
   ...
   */ /* eslint-disable valid-jsdoc */ /**
                                        ...
                                        */
const wrap$1 = window['ShadyDOM'] && window['ShadyDOM']['noPatch'] && window['ShadyDOM']['wrap'] ? window['ShadyDOM']['wrap'] : n => n;
var wrap$2 = {
  wrap: wrap$1
};
    ...
    for (let i = 0; i < path.length; i++) {
      if (path[i].nodeType === Node.ELEMENT_NODE) {
        if (path[i].localName === 'label') {
          ...
          let ownerLabels = matchingLabels( /** @type {!HTMLElement} */path[i]); // check if one of the clicked labels is labelling this element
...
document.addEventListener('touchend', ignoreMouse, SUPPORTS_PASSIVE ? {
  passive: true
             ...
             */
const getComposedPath = window.ShadyDOM && window.ShadyDOM.noPatch ? window.ShadyDOM.composedPath : event => event.composedPath && event.composedPath() || []; /** @type {!Object<string, !GestureRecognizer>} */
const gestures = {}; /** @type {!Array<!GestureRecognizer>} */
const recognizers = []; /**
...
function _findOriginalTarget(ev) {
  const path = getComposedPath( /** @type {?Event} */ev); // It shouldn't be, but sometimes path is empty (window on Safari).
...
function _fire(target, type, detail) {
  ...
  ev.detail = detail;
  wrap$1( /** @type {!Node} */target).dispatchEvent(ev); // forward `preventDefault` in a clean way
...
let FlattenedNodesObserver = class {
  ...
  connect() {
    if (isSlot(this._target)) {
      ...
      this._listenSlots( /** @type {!NodeList<!Node>} */wrap$1(this._target).children);
  ...
  disconnect() {
    if (isSlot(this._target)) {
      ...
      this._unlistenSlots( /** @type {!NodeList<!Node>} */wrap$1(this._target).children);
...
DomApiNative.prototype.querySelector; /**
                                       ...
                                       */
DomApiNative.prototype.querySelectorAll; /** @type {?Node} */
DomApiNative.prototype.parentNode; /** @type {?Node} */
DomApiNative.prototype.firstChild; /** @type {?Node} */
DomApiNative.prototype.lastChild; /** @type {?Node} */
DomApiNative.prototype.nextSibling; /** @type {?Node} */
DomApiNative.prototype.previousSibling; /** @type {?HTMLElement} */
DomApiNative.prototype.firstElementChild; /** @type {?HTMLElement} */
DomApiNative.prototype.lastElementChild; /** @type {?HTMLElement} */
DomApiNative.prototype.nextElementSibling; /** @type {?HTMLElement} */
DomApiNative.prototype.previousElementSibling; /** @type {!Array<!Node>} */
DomApiNative.prototype.childNodes; /** @type {!Array<!HTMLElement>} */
DomApiNative.prototype.children; /** @type {?DOMTokenList} */
DomApiNative.prototype.classList; /** @type {string} */
DomApiNative.prototype.textContent; /** @type {string} */
...
class TemplateInstanceBase extends templateInstanceBase {
  constructor(props) {
    ...
    this._configureProperties(props); /** @type {!StampedTemplate} */
...
function createTemplatizerClass(template, templateInfo, options) {
  ...
  let klass = class extends templatizerBase {}; /** @override */
...
class DomRepeat extends domRepeatBase {
  ...
  constructor() {
    ...
    this.__observePaths = null; /** @type {?function(new:Polymer.TemplateInstanceBase, *)} */
    this.__ctor = null;
...
class HomeView extends PageViewElement {
  ...
  firstUpdated() {
    ...
    const infoWindow = new google.maps.InfoWindow(); // Try HTML5 geolocation.
...
const minusIcon = html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M7 11v2h10v-2H7zm5-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;
const plusIcon = html$1`<svg height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0z" fill="none"/><path d="M13 7h-2v4H7v2h4v4h2v-4h4v-2h-4V7zm-1-5C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/></svg>`;
var myIcons = {
  menuIcon: menuIcon,
...
window.customElements.define('snack-bar', SnackBar); /* eslint-disable no-underscore-dangle */ /* eslint-disable import/prefer-default-export */
...
class Fetcher {
  /** ---- MAIN FETCHER FUNCTION - RUNS ALL API REQUESTS */ /**
                                                             ...
                                                             */static fetch(method, uri, body) {
    const requestInit = {
      method,
      body: JSON.stringify(body)
    };
    return fetch(`${baseUrl}${uri}`, requestInit).then(res => res.json()).then(data => data.data || data);
  } /** ---- CONSTRUCTOR ---- */ /**
                                  * @name constructor
...
class Services {
  /** ---- STATIC MEMBERS - RETURN INSTANCE OF FETCHER ---- */static get routes() {
...
class MyApp extends LitElement {
  ...
  constructor() {
    ...
    this._schools = [];
    this._selectedRoute = {};
    this._selecedSchool = {}; // To force all event listeners for gestures to be passive.
    // See https://www.polymer-project.org/3.0/docs/devguide/settings#setting-passive-touch-gestures

    setPassiveTouchGestures(true);
    Services.schools.getAll().then(res => {
      this._schools = res;
    });
  }
  ...
  _routeClicked(schoolId, routeId) {
    Services.routes.getRouteBySchoolId(schoolId, routeId).then(route => {
  ...
  _offlineChanged(offline) {
    ...
    this._offline = offline; // Don't show the snackbar on the first load of the page.
  ...
  _locationChanged(location) {
    ...
    this._page = page; // Close the drawer - in case the *path* change came from a link in the drawer.
